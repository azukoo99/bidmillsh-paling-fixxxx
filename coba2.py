...
def hapus_akun_kasir():
    ...
    while True:
        try:
            ...
            index_hapus -= 1
            if 0 <= index_hapus < len(akun_kasir):
                full_data = pd.read_csv('user.csv')
                full_data = full_data.drop(index=index_hapus + 1)
...
def laporan_keuntungan():
    while True:
        ...
        if pilihan == '1':
            ...
            filtered['tanggal'] = filtered['tanggal'].dt.date
            laporan = filtered.groupby('tanggal').agg({
                'keuntungan': 'sum'
            }).reset_index()
            laporan.index += 1
